...
export default function Dashboard({ user }: { user: any }) {
  ...
  const handleRandomJoin = async () => {
    try {
      ...
      const availableRooms = snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any) })) as Room[];
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {bgPresets.map(bg => (
                            <button
                              ...
                              className={`h-20 rounded-xl border-2 overflow-hidden transition-all ${newThemeBg === bg.url ? 'border-emerald-500 scale-105' : 'border-transparent opacity-60 hover:opacity-100'
                                }`}
                            >
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${room.status === 'waiting'
